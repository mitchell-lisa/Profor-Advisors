...
import { SiteHeader, SectionLabel, DataRail } from "@/components/chrome";
    ...
    <main id="top" className="flex-1">
      ...
      <section className="bg-navy text-white">
        <div className="mx-auto max-w-5xl px-6 sm:px-10">
          ...
          {/* Registration, above the fold, as figures. */}
          <DataRail />
        </div>
      </section>
      ...
      {/* Closing — the two things an allocator checks: who runs it, and is it
          registered. Side by side, each routing to its own page. */}
      <section className="border-t border-rule">
        <div className="mx-auto max-w-5xl px-6 py-16 sm:px-10 sm:py-20">
          <div className="grid gap-14 sm:grid-cols-2 sm:gap-12">
            {business.team.length > 0 && (
              <div>
                ...
                <p className="mt-8 font-serif text-xl leading-[1.45] sm:text-2xl">
                  {business.team[0].name}
                  <span className="block text-base text-muted sm:text-lg">
                    {business.team[0].role}
                  </span>
                </p>
                <p className="mt-8">
                  <a
